...
def plot_memory_use(history, fn_name, open_figure=True, offset=0, **kwargs):
    ...
    # Save plot to file
    save_path = f'logs/{fn_name}_memory.png'
    ...
    def meta_wrapper(fn):
        ...
        def wrapper(*args, **kwargs):
            ...
            with ThreadPoolExecutor() as executor:
                ...
                with open('logs/memory_usage.log', 'w', encoding='utf-8') as f:
                    f.write(f"{fn.__name__}: Current memory usage: {history[-1]:2f} MB\n")
                    f.write(f"{fn.__name__}: Peak memory usage: {max(history):2f} MB\n")
                    f.write(f"{fn.__name__}: Average memory used: {sum(history) / len(history):2f} MB\n")
                    f.close()
